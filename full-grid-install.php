 ...
 * 1. Refactor all occurrences of the name DT_Grid_Install, dt_grid_install, dt-grid-install and DT Grid Install with you're own
...
$dt_grid_install_required_dt_theme_version = '0.22.2';

/**
 * Gets the instance of the `DT_Grid_Install_Plugin` class.
 *
 * @since  0.1
 * @access public
 * @return object
 */
function dt_grid_install_plugin() {
...
class DT_Grid_Install_Plugin {
    ...
    public static function get_instance() {
        ...
        if ( is_null( $instance ) ) {
            $instance = new dt_grid_install_plugin();
            $instance->setup();
            $instance->includes();
...
//            );
    ...
    public static function deactivation() {
        delete_option( 'dismissed-dt-grid-install' );
    }
    ...
    public function i18n() {
        load_plugin_textdomain( 'dt_grid_install_plugin', false, trailingslashit( dirname( plugin_basename( __FILE__ ) ) ). 'languages' );
    }
    ...
    public function __clone() {
        _doing_it_wrong( __FUNCTION__, esc_html__( 'Whoah, partner!', 'dt_grid_install_plugin' ), '0.1' );
    }
    ...
    public function __call( $method = '', $args = array() ) {
        ...
        _doing_it_wrong( "dt_grid_install_plugin::{$method}", esc_html__( 'Method does not exist.', 'dt_grid_install_plugin' ), '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'DT_Grid_Install_Plugin', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'DT_Grid_Install_Plugin', 'deactivation' ] );

function dt_grid_install_plugin_hook_admin_notice() {
    global $dt_grid_install_required_dt_theme_version;
    $wp_theme = wp_get_theme();
    ...
    if ( ! get_option( 'dismissed-dt-grid-install', false ) ) { ?>
        <div class="notice notice-error notice-dt-grid-install is-dismissible" data-notice="dt-grid-install">
            <p><?php echo esc_html( $message );?></p>
        </div>
        <script>
            jQuery(function($) {
                $( document ).on( 'click', '.notice-dt-grid-install .notice-dismiss', function () {
                    $.ajax( ajaxurl, {
                        type: 'POST',
                        data: {
                            action: 'dismissed_notice_handler',
                            type: 'dt-grid-install',
